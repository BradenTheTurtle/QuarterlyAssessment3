...
# Function to fetch all table names (categories) from the database
def fetch_categories():
    ...
    categories = [table[0] for table in tables if table[0] != 'sqlite_sequence']
...
# If there are categories, populate the dropdown
if categories:
    category = tk.StringVar()
    category_menu = ttk.Combobox(main_window, textvariable=category, values=categories)
    category_menu.pack()
